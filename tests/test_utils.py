...
def test_filter_night_sun_multi_degrees():
    ...
    assert len(filtered_target) < len(target)
    assert len(filtered_datetimes) < len(datetimes)
    filtered_predictions2, filtered_target2, filtered_datetimes2 = filter_night(predictions=predictions, target=target,
                                                                                datetimes=datetimes, latitude=latitude,
                                                                                longitude=longitude,
    ...
    filtered_predictions3, filtered_target3, filtered_datetimes3 = filter_night(predictions=predictions, target=target,
                                                                                datetimes=datetimes, latitude=latitude,
                                                                                longitude=longitude,
                                                                                sun_position_for_night=5)
    assert len(filtered_target2) == len(filtered_datetimes2) == len(filtered_predictions2)
